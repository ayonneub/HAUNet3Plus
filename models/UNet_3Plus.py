...
class UNet_3Plus_DeepSup_CGM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #Classification using thresholding instead of argmax
        # cls_branch= self.cls(hd5).squeeze(3).squeeze(2)  # (B,N,1,1)->(B,N)
        # cls_confidence= cls_branch.max(dim=1)[0]
        # cls_branch_max= (cls_confidence > 0.3).float()
        # cls_branch_max= cls_branch_max[:, np.newaxis].float()

        #classification using softgating 
        # cls_branch= self.cls(hd5).squeeze(3).squeeze(2)
        # cls_confidence= cls_branch[:,1]
        # cls_branch_max= cls_confidence[:,np.newaxis]
